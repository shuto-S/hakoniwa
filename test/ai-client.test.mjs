...
test('最小間隔の間は生成しない(レート上限)', async () => {
  ...
  const c = new AiClient(enabled, backend, {
    now: () => now,
    limits: { maxPerDay: 100, minIntervalMs: 5000 },
  });
...
test('1日の上限を超えたら生成しない/翌日リセット', async () => {
  ...
  const c = new AiClient(enabled, backend, {
    now: () => now,
    limits: { maxPerDay: 2, minIntervalMs: 0 },
  });
...
test('ハードエラー(クォータ)でクールダウン+一度だけ通知', async () => {
  ...
  const c = new AiClient(enabled, backend, {
    now: () => now,
    limits: { minIntervalMs: 0, cooldownMs: 1000 },
  });
...
test('認証エラーは auth 種別で通知', async () => {
  ...
  const notices = [];
  const backend = makeBackend({
    generate: () => ({ ok: false, error: 'UNAUTHENTICATED: API key invalid' }),
  });
  const c = new AiClient(enabled, backend, {
    now: () => now,
    limits: { minIntervalMs: 0, cooldownMs: 1000 },
